...
class Snake():
    ...
    def draw(self):
        head_rect = pygame.Rect(int(self.body[0].x * cell_size), int(self.body[0].y * cell_size), cell_size - 1,
                                cell_size - 1)
...
def start():
    ...
    start_text_rect = start_text.get_rect(
        center=(int(cell_number * cell_size / 2), int(cell_number * cell_size / 2) + 50))
...
class Main():
    ...
    def body_collision(self):
        for block in self.snake.body[1:]:
            if block == self.snake.body[0] or (
                    not (0 <= self.snake.body[0].x <= 19) or not (0 <= self.snake.body[0].y <= 19)):
                game_over.play()
                self.game = False
                text = "You Lose!"
                self.win_lose_gui(text)
    ...
    def win_lose_gui(self, text):
            pygame.draw.rect(screen, ('#e5f2cd'), (200 , 300, 400, 300))
            font1 = pygame.font.SysFont('arial', 48)
            font2 = pygame.font.SysFont('arial', 30)
            comm = font1.render(text, True, (0,0,0))
            score_render = font2.render("Score: "+ str(main.score), True, (0,0,0))
            snakelenght_render = font2.render("Snake length:  "+ str(len(main.snake.body)), True, (0,0,0))
            screen.blit(comm, (315, 300))
            screen.blit(score_render, (210, 355))
            screen.blit(snakelenght_render, (210, 390))
    def check_win(self):
        if len(main.snake.body) >= 399:
            main.game = False
            text = "You Win!"
            self.win_lose_gui(text)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and main.snake.new_direction and main.game:
            ...
            if event.key == pygame.K_LEFT or event.key == pygame.K_a and main.snake.direction != Vector2(1,
                                                                                                         0) and main.snake.direction != Vector2(
                    -1, 0):
                main.snake.new_direction = False
                move_left.play()
                main.snake.direction = Vector2(-1, 0)

            if event.key == pygame.K_RIGHT or event.key == pygame.K_d and main.snake.direction != Vector2(-1,
                                                                                                          0) and main.snake.direction != Vector2(
                    1, 0):
                main.snake.new_direction = False
                move_right.play()
            ...
            if event.key == pygame.K_UP or event.key == pygame.K_w and main.snake.direction != Vector2(0,
                                                                                                       1) and main.snake.direction != Vector2(
                    0, -1):
                main.snake.new_direction = False
                move_up.play()
